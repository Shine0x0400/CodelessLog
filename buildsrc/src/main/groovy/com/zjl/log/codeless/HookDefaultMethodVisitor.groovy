...
class HookDefaultMethodVisitor extends AdviceAdapter {

    String methodName

    // for print only
    String methodLogName

    private String mClassName

    HookDefaultMethodVisitor(MethodVisitor mv, int access, String name, String desc, String className) {
        super(Opcodes.ASM6, mv, access, name, desc)
        methodName = name
        mClassName = className
        methodLogName = "${Logger.accCode2String(access)} ${methodName}${desc}"
        Logger.info("开始扫描方法：${methodLogName} in ${mClassName}")
    }
    ...
    void visitEnd() {
        super.visitEnd()
        Logger.info("结束扫描方法：${methodLogName} in ${mClassName}")
    }
